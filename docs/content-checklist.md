...
| ~~`public/about/team-photo.webp`~~ | **Real, 2026-07-30.** The group photograph, placed on `/about` between the quote and the statistics band after being declined for the `/team` hero — see the `PageHero` row. 1664x936, exactly 16:9 from a 1672x941 source, WebP q88, 160KB. It runs the container measure, not full bleed. **One defect in the supplied file:** the sign behind the group reads "BULLDNG COMMUNITIES", not "BUILDING" — legible on a high-density desktop screen. A corrected export would replace this file at the same path and dimensions with no code change. | Client (re-export) |
