...
**The placeholder generator was deleted on 2026-08-02** and its last two assets
are ordinary tracked files now: `public/map/static-map-placeholder.png` is the
map fallback until real map imagery lands, and
`content/projects/monterra-ridge/plans/plan-a.pdf` exists only as a test fixture
for the PDF resolution path in `tests/unit/project-media.test.ts`.
