...
map — and that is the complete list. The Monterra Bay hero was supplied on
2026-08-02, and the two Monterra Ridge floor plans stopped being a slot at all
the same day, when the block was dropped from that project's frontmatter.
Struck-through rows are done and are kept as a record of what was supplied and
...
| ~~`content/projects/monterra-ridge/plans/plan-a,b.png`~~ | **Block dropped 2026-08-02** — the second of the two options this row offered. Real drawings were never supplied: the seven-image set offered on 2026-07-30 was rejected on content (a seventeen-storey tower against copy that says forty-eight townhomes) and the 2026-08-02 set contained no plans, so `floorPlans` came out of Monterra Ridge's frontmatter and no project authors it now. **Nothing was deleted in code or on disk**: the schema field, `FloorPlanList`, the resolver and the two placeholder PNGs plus `plan-a.pdf` all remain, and `project-media.test.ts` still resolves those files directly, so re-authoring the frontmatter is all it takes to bring the section back. The plan alts stated 1,400 sq ft / three bedrooms and 2,100 sq ft / four bedrooms, which bracketed `sqftRange` — that pairing is gone with them, so `sqftRange` is now unstated anywhere else. | — |
